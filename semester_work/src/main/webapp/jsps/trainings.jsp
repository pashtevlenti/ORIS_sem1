...
<c:if test="${empty groups}">
    <c:if test="${hasCoach}">
        <p class="empty-message">Не найдено тренировок у тренера.</p>
    </c:if>
    <c:if test="${!hasCoach}">
        <p class="empty-message">Не найдено тренировок у спортсмена.</p>
    </c:if>
</c:if>
    ...
    <table class="groups-table">
        ...
        <c:forEach var="group" items="${groups}">
            <tr>
                ...
                <td>
                    <ul>
                        <c:forEach var="training" items="${group.get().trainings}">
                            <li>День недели - ${training.dayOfWeek}, время - ${training.time}
                                <c:if test="${hasCoach}">
                                    <div class="action-buttons">
                                        <form action="deleteTrainingFromGroup" method="post" style="display: inline;">
...
<c:if test="${hasCoach}">
    <div class="add-group-container">
        <form action="addTrainingToGroup" method="get">
            <button type="submit" class="add-group-btn">Добавить тренировку</button>
        </form>
    </div>
</c:if>
</body>
